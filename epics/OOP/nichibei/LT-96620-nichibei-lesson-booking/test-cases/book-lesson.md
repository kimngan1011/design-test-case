...
| --- | ---------------------------------------------------------- | ---------------------------------------------------------------------- | --------- |
| 1   | Open Lesson Lists and tap "Reserve" on an available lesson | Booking Confirmation Screen opens                                      | —         |
...
| #   | Action                                                      | Expected Result                                                              | Test Data |
| --- | ----------------------------------------------------------- | ---------------------------------------------------------------------------- | --------- |
| 1   | Open Lesson Lists and tap "Reserve" on the available lesson | Booking Confirmation Screen opens                                            | —         |
...
| 4   | Open My Lessons list                                        | The booked lesson appears in My Lessons list                                 | —         |
...
| 2   | Student A opens Lesson Lists, taps "Reserve" on the Draft lesson, then taps "Confirm" | Booking Success Screen shown                                  | —         |
...
| #   | Action                                                  | Expected Result                                             | Test Data                                 |
| --- | ------------------------------------------------------- | ----------------------------------------------------------- | ----------------------------------------- |
| 1   | Open Lesson Lists, tap "Reserve" on an available lesson | Booking Confirmation Screen opens                           | —                                         |
...
| #   | Action                                                                                            | Expected Result                                            | Test Data |
| --- | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- | --------- |
| 1   | Open Lesson Lists                                                                                 | The already-booked lesson is visible (Cancel button shown) | —         |
...
| #   | Action                                                         | Expected Result                                                       | Test Data    |
| --- | -------------------------------------------------------------- | --------------------------------------------------------------------- | ------------ |
| 1   | Open Lesson Lists                                              | Lesson card visible (may show as full)                                | capacity = 1 |
...
**Preconditions:**
Student's LA has expired between Lesson Lists and Confirmation step (or LA deleted mid-flow).
...
| #   | Action                                                            | Expected Result                                                       | Test Data                             |
| --- | ----------------------------------------------------------------- | --------------------------------------------------------------------- | ------------------------------------- |
| 1   | Open Lesson Lists and tap "Reserve" on a lesson that costs points | Booking Confirmation Screen opens                                     | remaining points = 0; lesson cost > 0 |
| 2   | Tap "Confirm"                                                     | Booking rejected                                                      | —                                     |
| 3   | View error message                                                | Error shown: "Remaining points are insufficient to book this lesson." | —                                     |

---

### [Nichibei] Lesson Booking – Validation – Rapid double-tap on Confirm – Only one Student Session created

**Description:** AC 03.2 — BR-16 / Client-side Idempotency: When a student taps the "Confirm" button rapidly multiple times during a single booking attempt (e.g., network latency causes student to re-tap thinking the first tap did not register), the server must process only one booking request and create exactly one Student Session. Duplicate in-flight requests must be silently discarded or rejected without creating a second session or charging points twice.

**Preconditions:**
Student A has NOT yet booked Lesson 1. Lesson 1 is bookable (Published, capacity ≥ 2, within deadline).
Student A has an active LA with sufficient points. No existing Student Session for (Student A, Lesson 1).

| #   | Action                                                                                          | Expected Result                                                                                 | Test Data           |
| --- | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | ------------------- |
| 1   | Student A opens Lesson Lists → taps Reserve on Lesson 1 → Booking Confirmation Screen opens     | Booking Confirmation Screen shown                                                               | Student A; Lesson 1 |
| 2   | Student A taps "Confirm" twice rapidly in quick succession (double-tap or re-tap within ~500ms) | App processes the first tap; second tap is ignored or blocked (button disabled after first tap) | capacity ≥ 2        |
| 3   | Observe the app result                                                                          | Booking Success Screen shown exactly once; no duplicate confirmation dialog or error loop       | —                   |
| 4   | Verify in Salesforce: Student Session count for (Student A, Lesson 1)                           | Exactly 1 Student Session exists with Booking_Flag=TRUE; no duplicate session created           | —                   |
| 5   | Verify lesson capacity count in Salesforce                                                      | Lesson capacity decremented by exactly 1 (not 2); points deducted exactly once from LA          | —                   |
...
### [Nichibei] Lesson Booking – Post-Booking – Lesson appears in My Lessons list

**Description:** AC 03.4 — BR-1: CRUD Testing: After a successful booking, the lesson appears in the student's My Lessons list.

**Preconditions:**
Student user has active LA. My Lessons list is initially empty.

| #   | Action                                          | Expected Result                                    | Test Data |
| --- | ----------------------------------------------- | -------------------------------------------------- | --------- |
| 1   | Confirm My Lessons list is empty before booking | My Lessons list shows empty state                  | —         |
| 2   | Book an available lesson via app                | Booking Success Screen shown                       | —         |
| 3   | Navigate to My Lessons list                     | The newly booked lesson appears in My Lessons list | —         |
...
| #   | Action                                                                         | Expected Result                                                               | Test Data           |
| --- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------- | ------------------- |
| 1   | Login as Student 1 → open Lesson Lists → tap Reserve on Lesson 1 → tap Confirm | Booking Success Screen shown for Student 1                                    | Student 1; Lesson 1 |
| 2   | Switch account to Student 2 on the same device                                 | Student 2 home screen shown                                                   | Student 2           |
| 3   | Open Lesson Lists → tap Reserve on Lesson 2 → tap Confirm                      | Booking Success Screen shown for Student 2                                    | Student 2; Lesson 2 |
...
| #   | Action                                                              | Expected Result                                                                                 | Test Data           |
| --- | ------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | ------------------- |
| 1   | Login as Student 1 → open Lesson Lists → Reserve Lesson 1 → Confirm | Booking Success Screen shown for Student 1                                                      | Student 1; Lesson 1 |
| 2   | Switch account to Student 2 on the same device                      | Student 2 home screen shown                                                                     | Student 2           |
| 3   | Open Lesson Lists → Reserve Lesson 1 → Confirm                      | Booking Success Screen shown for Student 2                                                      | Student 2; Lesson 1 |
...
| #   | Action                                                            | Expected Result                                                | Test Data       |
| --- | ----------------------------------------------------------------- | -------------------------------------------------------------- | --------------- |
| 1   | Login as Student → open Lesson Lists → Reserve Lesson 1 → Confirm | Booking Success Screen shown; Student Session created          | Student account |
...
| #   | Action                                                        | Expected Result                                                       | Test Data         |
| --- | ------------------------------------------------------------- | --------------------------------------------------------------------- | ----------------- |
| 1   | Login as Parent → navigate to home screen                     | Parent home screen shown; Reservation List menu NOT visible           | Parent account    |
| 2   | Switch account to Student on the same device                  | Student home screen shown                                             | Student account   |
| 3   | Open Lesson Lists → Reserve Lesson 2 → Confirm                | Booking Success Screen shown                                          | Student; Lesson 2 |
...
| 2   | Login as Student A in Learner App → open Lesson Lists                              | Lesson 1 is visible in Lesson Lists (Bookable Flag=TRUE)    | —                   |
...
| 2   | Login as Student A → Lesson Lists → Reserve Lesson 1 → Confirm                       | Booking Success Screen shown                            | Student A; Lesson 1             |
...
## Suite: Cross-Midnight – Stale UI

### [Nichibei] Lesson Booking – Cross-Midnight – Lesson visible at 23:58 JST – Crosses midnight – Reserve blocked by server

**Description:** AC 02.2 — BR-7 / Stale UI: When a student loads Lesson Lists just before midnight (23:58 JST on 2026-05-19) and a lesson has lesson_date=2026-05-20 (tomorrow = today+1 ≥ threshold X=1 → visible), then after midnight passes (now 2026-05-20 00:01 JST), that same lesson_date=2026-05-20 becomes "today" and falls below the threshold. The UI still shows Reserve enabled (stale cache), but the server must reject the booking.

**Preconditions:**
Today = 2026-05-19. Partner config: X = 1. Lesson TZ = UTC+9 (JST).
Lesson A: lesson_date=2026-05-20, Bookable_Flag=TRUE, Published, capacity ≥ 1, within booking deadline.
Student has active LA for lesson's location.

| #   | Action                                                                                            | Expected Result                                                    | Test Data                                                               |
| --- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ | ----------------------------------------------------------------------- |
| 1   | At 23:58 JST (2026-05-19), open Lesson Lists                                                      | Lesson A (2026-05-20) is visible; Reserve button enabled           | Time: 23:58 JST 2026-05-19; lesson_date=2026-05-20; X=1                 |
| 2   | Wait without refreshing until 00:01 JST (midnight passes, now 2026-05-20)                         | UI still shows Lesson A with Reserve enabled (stale cache)         | New server date: 2026-05-20; lesson_date=2026-05-20 = today → below X=1 |
| 3   | Tap Reserve on Lesson A → Booking Confirmation Screen opens (stale UI still shows lesson details) | Confirmation Screen shown (opened from stale Lesson Lists state)   | —                                                                       |
| 4   | Tap Confirm                                                                                       | Booking request sent to server                                     | —                                                                       |
| 5   | View server response                                                                              | Booking rejected — lesson no longer meets advance-days requirement | —                                                                       |
| 6   | Verify no Student Session created in Salesforce for (Student, Lesson A)                           | No session created; Student A's My Lessons unchanged               | —                                                                       |
...
## Suite: Post-Booking – Staff View & Data Integrity

### [Nichibei] Lesson Booking – Post-Booking – LA Lesson Allocated count increments by 1 after booking

**Description:** AC 03.2 — BR-12: Data Integrity: When a student books a lesson via app (creating a Student Session with Booking_Flag=TRUE), the linked LA's Lesson_Allocated count must increment by 1 and the LA status must update accordingly. This mirrors the same counter update triggered by manual staff assignment.

**Preconditions:**
Student A has exactly 1 active LA (LA-A). LA-A's Lesson_Allocated = 0; LA Status = "None Assigned".
A bookable Published lesson exists at LA-A's location, within deadline, capacity ≥ 1.

| #   | Action                                                          | Expected Result                                                             | Test Data                                |
| --- | --------------------------------------------------------------- | --------------------------------------------------------------------------- | ---------------------------------------- |
| 1   | Record LA-A Lesson_Allocated count in Salesforce before booking | LA-A Lesson_Allocated = 0; Status = "None Assigned"                         | LA-A Lesson_Allocated = 0 before booking |
| 2   | Student A books the lesson via app (tap Reserve → Confirm)      | Booking Success Screen shown                                                | —                                        |
| 3   | Verify LA-A Lesson_Allocated count in Salesforce after booking  | LA-A Lesson_Allocated = 1 (incremented from 0)                              | —                                        |
| 4   | Verify LA-A status in Salesforce after booking                  | LA Status updated to "Partial Assigned" or "Fully Assigned" per total count | —                                        |

---

### [Nichibei] Lesson Booking – Post-Booking – Student appears in BO Lesson Detail student tab after booking

**Description:** AC 03.2 — BR-12: Cross-system: After a student books a lesson via the Learner App, staff must be able to see that student listed in the BO Lesson Detail student tab. The booking creates a Student Session which populates the staff-visible student list.

**Preconditions:**
Student A has active LA. Lesson 1 is bookable (Published, capacity ≥ 1, within deadline). No students currently in Lesson 1 student tab.
Staff account has BO access.

| #   | Action                                                     | Expected Result                                                          | Test Data           |
| --- | ---------------------------------------------------------- | ------------------------------------------------------------------------ | ------------------- |
| 1   | Staff opens BO → navigates to Lesson 1 → opens Student tab | Student tab is empty (no students assigned yet)                          | —                   |
| 2   | Student A books Lesson 1 via app (tap Reserve → Confirm)   | Booking Success Screen shown                                             | Student A; Lesson 1 |
| 3   | Staff refreshes BO Lesson 1 Student tab                    | Student A appears in the student list                                    | —                   |
| 4   | Verify Student A's entry details in the student tab        | Student A's name visible; attendance status shows the booking assignment | —                   |

---

### [Nichibei] Lesson Booking – Post-Booking – Lesson Report Detail auto-created for booked student

**Description:** AC 03.2 — BR-12: Data Integrity: When booking creates a Student Session, a Lesson Report Detail record must be auto-created for (Student, Lesson) in Salesforce. This is the same record created during manual staff assignment and is required for the Collect Attendance workflow.

**Preconditions:**
Student A has active LA. Lesson 1 is bookable (Published, capacity ≥ 1, within deadline).
No Lesson Report Detail record exists for (Student A, Lesson 1) prior to booking.

| #   | Action                                                                        | Expected Result                                                           | Test Data           |
| --- | ----------------------------------------------------------------------------- | ------------------------------------------------------------------------- | ------------------- |
| 1   | Verify no Lesson Report Detail exists for (Student A, Lesson 1) in Salesforce | No record found for (Student A, Lesson 1)                                 | Student A; Lesson 1 |
| 2   | Student A books Lesson 1 via app (tap Reserve → Confirm)                      | Booking Success Screen shown                                              | —                   |
| 3   | Verify Lesson Report Detail in Salesforce after booking                       | A Lesson Report Detail record exists for (Student A, Lesson 1)            | —                   |
| 4   | Verify Lesson Report Detail fields                                            | Record is linked to the correct Student, Lesson, and LA; status = Not Yet | —                   |

---

### [Nichibei] Lesson Booking – Post-Booking – Auto-published Draft lesson becomes visible to other assigned students on Mobile

**Description:** AC 03.2 — BR-13: Cross-system: When booking auto-publishes a Draft lesson (lesson transitions Draft → Published), all other students already assigned to that lesson must immediately see it in their Lesson List on the Learner App. Draft lessons are hidden from students; Published lessons are visible.

**Preconditions:**
Lesson 1 is Draft, Bookable_Flag=TRUE, capacity ≥ 2. Both Student A and Student B are assigned (have a Student Session) for Lesson 1.
Student B's Learner App: Lesson 1 is NOT visible (Draft = hidden). Student A will perform the booking.

| #   | Action                                                   | Expected Result                                                                                  | Test Data                    |
| --- | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ---------------------------- |
| 1   | Confirm Lesson 1 status = Draft in Salesforce            | Lesson 1 status = Draft                                                                          | Lesson 1 = Draft             |
| 2   | Student B opens Lesson List on app                       | Lesson 1 is NOT visible to Student B (Draft lessons hidden from students)                        | Student B; Lesson 1 is Draft |
| 3   | Student A books Lesson 1 via app (tap Reserve → Confirm) | Booking Success Screen shown; lesson auto-published silently                                     | Student A books Lesson 1     |
| 4   | Verify Lesson 1 status in Salesforce after booking       | Lesson 1 status = Published                                                                      | —                            |
| 5   | Student B refreshes Lesson List on app                   | Lesson 1 is NOW visible to Student B (Published lesson becomes visible to all assigned students) | —                            |

---

### [Nichibei] Lesson Booking – Post-Booking – Staff can mark attendance for student who booked via app

**Description:** AC 03.2 — BR-12: End-to-end integration: After a student books via app (creating Student Session + Lesson Report Detail), staff must be able to open BO Collect Attendance, locate the student in the list, select an attendance status, save, and have that status persisted. This verifies the full booking-to-attendance pipeline unique to the Nichibei booking flow.

**Preconditions:**
Student A has booked Lesson 1 via app (Booking_Flag=TRUE). Lesson Report Detail auto-created for (Student A, Lesson 1).
Lesson 1 is Published. Staff has BO access with Collect Attendance permission.
No attendance status recorded for Student A on Lesson 1 yet.

| #   | Action                                                                           | Expected Result                                                                                                                            | Test Data           |
| --- | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------- |
| 1   | Staff opens BO → Lesson List → clicks Collect Attendance entry point on Lesson 1 | Collect Attendance page opens; Student A is listed in the student list (booking created the Lesson Report Detail that populates this list) | Student A; Lesson 1 |
| 2   | Observe Student A's row in Collect Attendance                                    | Student A visible; attendance status = Not Yet; "Booking Note: [remark]" shown if remark was entered at booking time                       | —                   |
| 3   | Staff selects "Attend" for Student A's row                                       | "Attend" radio button selected for Student A; no error                                                                                     | Status = Attend     |
| 4   | Staff clicks Save                                                                | Save success; confirmation shown or page refreshes without error                                                                           | —                   |
| 5   | Staff re-opens Collect Attendance for Lesson 1                                   | Student A's attendance status = "Attend" (persisted correctly)                                                                             | —                   |

---

### [Nichibei] Lesson Booking – Post-Booking – Student submits attendance response for booked lesson via app

**Description:** AC 03.2 / Submit Attendance Flow: After booking a lesson, a student can separately submit an attendance response (remark or status) for that lesson via the Learner App. This "Submit Attendance" response is saved without the "Booking Note: " prefix — it is a distinct field update from the original booking remark and must be visible to staff in BO Collect Attendance.

**Preconditions:**
Student A has booked Lesson 1 (Booking_Flag=TRUE). Lesson 1 is upcoming and Published.
Student A did NOT enter a remark at booking time (booking remark is blank).

| #   | Action                                                                                   | Expected Result                                                                                                       | Test Data                              |
| --- | ---------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| 1   | Student A opens Learner App → navigates to Lesson List → locates Lesson 1                | Lesson 1 is visible; shows "Cancel" button (booked lesson)                                                            | Student A; Lesson 1                    |
| 2   | Student A opens the Submit Attendance flow for Lesson 1 (tap attendance response option) | Attendance response screen opens with a remark input field                                                            | —                                      |
| 3   | Student A enters remark: "Will arrive 10 minutes late" and submits                       | Submission confirmed; no error shown                                                                                  | remark = "Will arrive 10 minutes late" |
| 4   | Verify Attendance_Response_Remark in Salesforce for (Student A, Lesson 1)                | Field value = "Will arrive 10 minutes late" — NO "Booking Note: " prefix (Submit Attendance flow, not booking remark) | —                                      |
| 5   | Staff opens BO Collect Attendance for Lesson 1 → locates Student A's row                 | Student A's remark field shows "Will arrive 10 minutes late" without "Booking Note: " prefix                          | —                                      |
