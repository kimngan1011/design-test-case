...
- **New BO entry points for Collect Attendance (LT-96152, 2026-04-15):**
  - Entry point 1: BO Lesson Detail → **Report tab** → "Collect Attendance" button
  - Entry point 2: **BO Lesson Report Detail** page → "Collect Attendance" button
  - Both open the **existing** collect-attendance popup (same UI, same save pipeline as original entry point)
  - Both are **disabled** for Draft and Completed lessons (same gate as the original entry point)
  - Saving from either new surface updates the same data: student-session, lesson allocation/report-history, and Mobile-facing attendance values
  - Reopening the popup from either new surface shows previously saved attendance values (reason, notice, note pre-filled)
...
- [Nichibei] Students can self-book and self-cancel lessons via the Lesson Booking menu (feature-flag controlled; **Parents have NO access**)
...
> **⚠️ Timezone risk:** Month boundary must use the lesson date as displayed in the UI (local timezone), NOT the raw UTC stored value. A lesson at 11:30pm JST (Jan 31) is stored as UTC Feb 1 — it must be counted in January. Implementation must derive the month from the UI local date. Added from LT-96673 (2026-04-16).

---

### 2.1.3 Lesson Publish Notifications (Renseikai — LT-96662, 2026-04-15)

> **See also:** Section 2.1.4 for Nichibei Lesson Booking System — auto-publish on booking is **silent** (no notification triggered, unlike this feature).
...
### 2.1.4 Lesson Booking System (Nichibei — LT-96620, 2026-05-05)
...
- Uses the **same point-deduction mechanism as manual staff assignment** in Nichibei
...
- These are SF-only notifications (not push); triggered by app actions only, NOT by staff SF assignments
...
| **Renseikai/Rensei** | Collect attendance on SF (2) and BO (2), Configure error messages (15), Bulk update attendance (34), Class name uniqueness (8), Collect Attendance new entry points — Report tab + Lesson Report Detail (LT-96152), Publish and notify student (LT-96662) |   61+ |
...
| **Riso**             | Lesson Allocation management (80), Subject in Lesson Detail (25), Monthly Lesson Count in Add Teacher popup (LT-96673)                                                                                                                                    |   105 |
